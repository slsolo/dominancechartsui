...
export default ({
  ...
  methods: {
    async fetchCompare() {
      this.loading = true;
      try {
        const ey = await this.$axios.$get(`${process.env.API_HOST}api/Tails/compare/${this.first}/${this.second}`);
        this.result = ey;
      } catch (e) {
        this.error = e;
      } finally {
  ...
  async fetch() {
    this.tails = await fetch(`${process.env.API_HOST}api/Tails/names`, {
      method: 'get'
    }).then(res => res.json())
  }
});
